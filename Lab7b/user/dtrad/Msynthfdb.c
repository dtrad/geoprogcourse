...
static float **v0,**vv, **p0, **p1, **ptr=NULL, **vel;
...
int main(int argc, char* argv[])
{
	...
	sf_warning("number of threads is %d, CLOCKS_PER_SEC is %d",nthreads,CLOCKS_PER_SEC);
	...
	for(is=0; is<ns; is++)
	{
		...
		time(&end2);
		seconds = difftime(end2,start2);
 		sf_warning("shot %d finished (clock ticks): %f", is+1,((float)(end-start))/CLOCKS_PER_SEC); 
 		sf_warning("shot %d finished: %f", is+1,seconds); 
