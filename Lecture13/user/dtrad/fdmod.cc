...
Fdmodb::Fdmodb(int nx, int nz, int nt, int nb, int nbup, int ng, int ns, float dx, float dz, float dt, float* vel, float fm, float amp, int order){
  ...
  else if (order == 8) myEightOrder = true;
  else cerr << "method not implemented = " << order << endl;
  cerr << "FD with " << order << " in space and 2nd in time " << endl;
...
void Fdmodb::stepForward(){
  ...
  else if (myEightOrder)  stepForwardEO(myP0,myP1);
}
...
void Fdmodb::transposeAdd(float* trans){
  int i1, i2; 
  for(i2=0; i2<myNt; i2++)
    for(i1=0; i1<myNg; i1++)
      trans[i2+myNt*i1]+=myDObs[i1+myNg*i2];
}
...
bool Fdmodb::isStable(float* velp){
  ...
  float vmin=vmax;
  for (size_t i=0;i<n;i++) vmin=min(vmin,velp[i]);
  cerr << " vmin = " << vmin << endl;
  return stable;
