...
/* Interface to several solvers (wtcgls, toep, cgfft)

   This function solves the system of equations 
     (FH WdT Wd FH + WmT Wm ) m = FH WdT Wd d 
   Notice that LH=FH WdT and L= Wd F are computed with matrix_3.
      
   If noise and model are uncorrelated then data and model covariances 
   matrices Cd and Cm  are diagonal and can be factorize by simple sqrt. 
   ...
   Wm is used to increase resolution (model preconditioner)
   Wd is used to taper data at the boundaries and 
      to reduce effect of killed or bad traces

   Wm depends on the model and is updated in external iterations 
   Wd does not change. 
...
void radonsolver(float *pos, int nh, float **data, float *t, int nt, float dt,float **model, float *q, int nq, float dq, float eps1, float eps2, float eps, float fmax, float *Wd, int itercg, int iter_end,int norm,float step, int testadj, int rtmethod, float depth, char *solver)
{
  ...
  float *Cd;
   ...
   // su alloc functions require reversing dimension
   ...
   // a possible way to calculate data covariance uses position
   //for (ih=0;ih<nh;ih++) Cd[ih]=Wd[ih]*nh/fabs(pos[nh-1]-pos[0]);
   //for (ih=0;ih<nh;ih++) Wd[ih]=Wd[ih]*nh/fabs(pos[nh-1]-pos[0]);
   
   
   // different types or RT (rtmethod) use different functions of offset.
