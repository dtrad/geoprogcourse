...
int main(int argc, char **argv) {
    ...
    segy tr;
    int LI=1; // Linear interpolation LI=1, sinc LI=0
    ...
    for (int ix = 0; ix < nx; ix++) { // loop over midpoints
	    ...
	    else{ // for more precision this can be replaced by ints8r (sinc interpolation)
	      float* ttn=ealloc1float(nt);
	      float* tnt=ealloc1float(nt);
	      float* model=ealloc1float(nt);
		...
		int t0=tau0/dt;
              ...
              if (1){
                ...
                tnt[0]=0;
                ints8r(nt,1.0,0.0,model,0,0,nt,tnt,data);
              }
