...
//   Modified from sflsprtm for geophysical programming course.  Daniel Trad - 2017-University of Calgary
//  
...
{   
    ...
    if (!haveImage) { // if not given calculate
      ...
      // create RTM for comparison
      rtm->loop(true,false,&modb[0],&dat[0]);
      
      if (niter){
	cerr << "**** Start LS migration " << endl;
    ...
    // write contents of mod to output files.
    imag << mod;   // LSRTM
    imagb << modb; // RTM
