...
private:
  ...
  void stepForward(float** p0, float** p1, bool adj, 
		   float c0, float c11, float c12, float c21, float c22);
  void applySponge();
  ...
  // additional functions to save wavefield combinations
  float** difference(float** p, float** g);
  float** scatter(float** s, float* m);
  void scatter(float* m, float** s, float **g, bool adj);
