...
void RtmClass::stepForwardSO(float** p0, float** p1, bool adj)
/*< forward modeling step second order>*/
{
  if (adj) cerr << " not implemented yet" << endl;
...
void RtmClass::loop(bool adj, bool add, float* model, float* data){
  ...
  for (int is=0;is<myNs;is++){
	...
	//stepForward(myGP0,myGP1,false, c0,c11,c12,c21,c22);
	...
	addSource(is,it,true);
	stepForward(mySP0,mySP1,false); // notice is false here
	ptr=mySP0;mySP0=mySP1;mySP1=ptr;
	applySponge(mySP0);
	applySponge(mySP1);
	//if (is == 0) saveWave(it,mySP0);
	imaging(myModTmp,mySP0,myGP1,adj);
	if (is == 0) saveWave(it,myGP1);
	rcvrTimeSlice(myGP1,data,is,it,adj);
	stepForward(myGP0, myGP1, true); // true is forward
	//stepForward(myGP0, myGP1, true, c0,c11,c12,c21,c22); // true is forward
	ptr=myGP0; myGP0=myGP1; myGP1=ptr;
	applySponge(myGP1);
	applySponge(myGP0);
      }
