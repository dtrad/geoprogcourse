...
/* Function to compute and subtract the multiples generated after muting the primaries 
   in the RT space. There are two routines because the forward operator can take 
   different shapes but the only difference is the forward operator used. 
		    ...
		    float *q, float *h, int itm, int plot)
{
  ...
  side=1;
  iq=0; while((q[iq]<parmute)&&(iq<nq)) iq++; 
		    ...
		    int nh, int nq, int nsparse, float parmute, float *q, float *h, int itm, int plot)
{
  ...
  if ((plot==2)||(plot==3)){
    ...
    save_gather(modeltemp,nq,q,nt,0.004,"mutedmodel.su");
    system("suxwigb < mutedmodel.su perc=99 key=f2 title=mutedmodel ");
  }
  ...
  if ((plot==2)||(plot==3)){
    ...
    save_gather(datarec,nh,h,nt,0.004,"multiples.su");
    system("suxwigb < multiples.su perc=99 key=offset  title=multiples ");
  }
