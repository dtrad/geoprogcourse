...
char *sdoc[] = {
  " 	   								",
  " SURADONTIME  Time domain hyperbolic High Resolution Radon Transform ",
  " 	   								",
  " suradontime < stdin > stdout [optional parameters]       		",
  "                                                                     ",
  " Multiple attenuation by time domain hyperbolic Radon transform      ",
  " Input must be sort by cdp and offset                                ",
  "                                                                     ",
  ...
  " dperv=0         defines the rate of increasing velocity spacing     ",
  " nq=100          number of traces in Radon space                     ",
  " itercg=10       internal iterations for CG                          ",
  " iter_end=1      external iterations for CG                          ",
  " eps1=1e-3       numerator hyper-parameter for Wm                    ",
  " eps2=1e-3       denominator  hyper-parameter for Wm                 ",
  " mute=0          =1 mute multiples                                   ",
  " velmute=0       minimum velocity in mute, if not given use parmute  ",
  " parmute=1e-8   if q > parmute the radon trace is tapered            ",
  " verbose=0       =1  verbose output                                  ",
  " t0=0            First useful time                                   ",
  " tm=0            The time for the first  multiple to be subtracted   ",
  " plot=0          =1 =>      plot Radon space and primaries           ",
  "                 =2 =>      plots after mute                         ",
  " centralq=nq/2  the q trace with the given velocities                ",
  ...
  "                                                                     ",
  " Examples: 		                                                ",
  "  suradontime < data.su par=stkvel.data.su itercg=25 iter_end=3      ",
  "                 modelfile=modelsp.su > dataout.su                   ",
  "                                                                     ",
  "  suradontime < filein.su  cdpmin=1 cdpmax=1000 dxcdp=1              ",
  "  nhcdp=200 par=stkvel.filein.su itercg=25 iter_end=1 eps2=1e-1      ",
  "  eps1=1e-3 nq=100 verbose=1 ntrmax=200                              ",
  "  norm=0 t0=0 mute=1 velmute=1500 LI=1 nw=21 fpeak=45                ",
  "  centralq=50 pervmin=1e-7 smute=2 typewav=2 plot=1 > fileout.su     ",
  "                                                                     ",
  NULL};
/* Credits:
...
int main(int argc, char **argv)
{
  ...
  int dataprec;  // dataprec =0   event with energy higher than quantile 99 are  
                 // downweighted by Q50/Q99                           
  ...
  if (!getparfloat("velmute",&velmute)) velmute = 0; // if not given use parmute
  if (!getparfloat("parmute",&parmute)) parmute=4.3e-7;
  if (velmute) parmute=1./(velmute*velmute);
  ...
  // velocity grid design parameters.
  if (!getparfloat("pervmin",&pervmin)) pervmin = 4e-9;
  if (!getparfloat("dperv",&dperv)) dperv =0.;

  if (!getparint("centralq",&centralq)) centralq=nq/2;
  if (!getparint("dataprec",&dataprec)) dataprec =0;
  if (!getparint("nw",&nw)) nw =11;
  if (!getparfloat("fpeak",&fpeak)) fpeak =45;
  if (!getparint("typewav",&typewav)) typewav = 2;
  ...
  for (ix=0;ix<nx;ix++){
   ...
   /**************************************************/
    ...
    radontd_sparse(t,q,h,mm,cmp,nt,nhcmp[ix],nq,dt,velint,dperv,pervmin,t0,inv,
		   centralq,dataprec,nw,fpeak,typewav,LI,parmute,mute,itm,plot);
    // Output trace
    //erewind(tracefp);
    erewind(headerfp);  
